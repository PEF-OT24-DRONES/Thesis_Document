...
\subsection{ROS 2 Architecture}
    ...
    The combination of these elements (diagram showing these connections in Figure \ref{fig:ros2_architecture}) allows robotic systems designed in ROS 2 to be highly scalable and flexible, adapting to both small projects and complex systems.  
    ...
    The architecture of ROS 2 brings several advantages that make it ideal for modern distributed robotic systems. The key benefits are shown in Table \ref{table:benefits} \cite{ros_docs}:  
...
\subsection{Computer Vision}
    ...
    Calibration involves determining intrinsic parameters (such as focal length and principal point) and extrinsic parameters (relative to the camera's position and orientation) that map 2D coordinates from captured images to real-world 3D coordinates. Example images of camera distortion in Figure \ref*{fig:distortions}.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \subcaption{ArUco without pose estimation.} 
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \subcaption{ArUco with pose estimation.}
        \label{fig:aruco_pose_estimation}
    \end{minipage}
    \caption{ArUco markers: (a) without pose estimation, (b) with pose estimation.}
    \label{fig:aruco_comparison}
\end{figure}


Figures \ref{fig:aruco_no_pose_estimation} and \ref{fig:aruco_pose_estimation} illustrate different uses of ArUco markers in the context of computer vision applications. 

Figure \ref{fig:aruco_no_pose_estimation} shows an example of ArUco markers without pose estimation. In this scenario, the markers are used for basic detection and identification without providing information about their orientation or position in 3D space.

Figure \ref{fig:aruco_pose_estimation} demonstrates the use of ArUco markers with pose estimation. Here, the markers not only identify the presence of the markers but also calculate their pose, giving information about their rotation and translation in three dimensions.
