...
\subsubsection{Components and CAD Designs}
    \begin{enumerate}
        ...
        \item 12V Motor Mounts
            ...
            \hfill
            \end{figure}
        \item V-Belt Holder for the internal drawer
            \begin{figure}[h]
            \centering
            \begin{minipage}{0.45\textwidth}
                \centering
...
The system was designed to detect ArUco markers in real-time using images captured by a camera attached to the drone. This process was carried out via a ROS 2 node that processes images and estimates the pose of the markers relative to the camera. The full code is included in the appendices of the document. Below is the step-by-step description of the system:
    ...
    \begin{enumerate}
        ...
        \begin{center}
            \begin{figure}[h!]
                \centering
                %\includegraphics[width=0.8\textwidth]{pictures/markers_axes.png}
        ...
        The results of ArUco marker detection were used to visualize the drone's position and orientation in real-time, enabling the control system to adjust these parameters for landing on the charging station.
    \end{enumerate}
...
\section{Communication System}
    ...
    \begin{itemize}
        \item \textbf{WiFi-Based Local Network:} 
        Provide effective communication between devices without requiring Internet access.
        \item \textbf{ROS 2 DDS Middleware:} 
        Use ROS 2's node and topic architecture to manage data communication between connected devices.
    \end{itemize}
    
    \subsection{Communication Structure} 
    
    \subsubsection{WiFi Local Network} 
    ...
    \begin{figure}
        ...
        \caption{Diagram of the WiFi local network used in the system.}
    \end{figure}
    
    \subsubsection{Communication via ROS 2} 
    ...
    \begin{itemize}
        \item \textbf{Drone:} 
        Published camera images (\texttt{/camera\_image/compressed}) and status data related to battery, position, and other critical parameters.
        \item \textbf{Ground Computer:} 
        Contains an interface that subscribed to topics published by the drone to:
        \begin{itemize}
            \item Process images and calculate the position of ArUco markers.
            \item Display drone status data, such as battery level and flight mode, in real-time.
            \item Publish commands to the charging station, such as opening or retracting the drawer, via specific topics.
        \end{itemize}
        \item \textbf{Charging Station:} 
        Subscribed to commands sent from the ground computer's interface to execute necessary actions, such as opening and closing the drawer.
    \end{itemize}
    
    \begin{figure}
        \centering
        %\includegraphics[width=0.8\textwidth]{pictures/ros2_comms.png}
        \caption{Communication diagram between devices using ROS 2.}
    \end{figure}
